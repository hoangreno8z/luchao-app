...
export function calculateSolarTermDate(year: number, termIndex: number): Date {
...
export function getSolarTerm(year: number): Date[] {
  const termInfo: Date[] = [];
  for (let i = 0; i < 24; i++) {
...
export function calculateCanChi(dateInput: string | Date): CalendarData {
  ...
  const solarYear = d < lapXuan ? y - 1 : y;
  ...
  if (d >= termsPrev[22] && d < terms[0]) {
    ...
    const mapping: Record<number, number> = { 2: 2, 4: 3, 6: 4, 8: 5, 10: 6, 12: 7, 14: 8, 16: 9, 18: 10, 20: 11, 22: 0, 0: 1 };
    for (const tIdx of checkOrder) {
      if (d >= terms[tIdx]) {
  ...
  const getNapAm = (can: string, chi: string): string => {
    const key = `${can} ${chi}`;
