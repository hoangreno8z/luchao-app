import { CalendarData, CAN, CHI, NGU_HANH_CHI } from './calendarCore';

export interface PhucThanData {
  rel: string;
  branch: string;
  can: string;
  isActive: boolean;
...
export interface DateInfo {
  ...
  nhatThan: string;
  nguyetLenh: string;
  nhatLenhShort: string;
  nguyetLenhShort: string;
  shenshaRaw: string[];
}
...
export const QUAI_SO: QuaiSoItem[] = [
...
export const NAP_GIAP: Record<string, string[]> = {
...
export const LUC_XUNG_LIST = ['Thiên Lôi Vô Vọng', 'Lôi Thiên Đại Tráng'];
export const LUC_HOP_LIST = [
...
export function getHexAttribute(hexName: string, type: string): string {
...
export const PHAN_NGAM_PAIRS: Record<number, number> = {
  7: 3, 3: 7, 5: 2, 2: 5, 4: 6, 6: 4, 1: 0, 0: 1
};

export const PHUC_NGAM_PAIRS: Record<number, number> = {
  7: 4, 4: 7
};

export function checkNgam(mainInIdx: number, mainOutIdx: number, changedInIdx: number, changedOutIdx: number): string[] {
  let noiResult = '';
  let ngoaiResult = '';
...
const HEX_MAP: Record<number, HexagramInfo> = {};

function initHexMap(): void {
  const add = (o: number, i: number, p: number, shi: number, t: string) => {
...
export const LIFE_STAGES = ['T.Sinh', 'M.Dục', 'Q.Đới', 'L.Quan', 'Đ.Vượng', 'Suy', 'Bệnh', 'Tử', 'Mộ', 'Tuyệt', 'Thai', 'Dưỡng'];
export const LS_START: Record<string, number> = { 'Hỏa': 2, 'Kim': 5, 'Mộc': 11, 'Thủy': 8, 'Thổ': 8 };

export const LUC_THU: Record<string, string[]> = {
...
export function getBit(val: number, changed: boolean): string {
  if (!changed) {
...
export function getRelation(el: string, palaceEl: string): string {
...
export function getLifeStage(el: string, baseChi: string): string {
  const start = LS_START[el] ?? 0;
  const current = CHI.indexOf(baseChi);
  const diff = (current - start + 12) % 12;
  return LIFE_STAGES[diff];
}

export function calculateShenSha(dCan: string, dChi: string, mChi: string): string[] {
  const list: string[] = [];
  const add = (name: string, val: string | string[] | undefined) => {
    let vStr = val || '';
  ...
  const quy: Record<string, string[]> = {
  ...
  const kinh: Record<string, string> = { 'Giáp': 'Mão', 'Ất': 'Dần', 'Bính': 'Ngọ', 'Mậu': 'Ngọ', 'Đinh': 'Tỵ', 'Kỷ': 'Tỵ', 'Canh': 'Dậu', 'Tân': 'Thân', 'Nhâm': 'Tý', 'Quý': 'Hợi' };
  add('Dương Nhận', kinh[dCan]);

  const van: Record<string, string> = { 'Giáp': 'Tỵ', 'Ất': 'Ngọ', 'Bính': 'Thân', 'Mậu': 'Thân', 'Đinh': 'Dậu', 'Kỷ': 'Dậu', 'Canh': 'Hợi', 'Tân': 'Tý', 'Nhâm': 'Dần', 'Quý': 'Mão' };
  ...
  if (group) {
    const dm: Record<string, string> = { 'Thủy': 'Dần', 'Hỏa': 'Thân', 'Kim': 'Hợi', 'Mộc': 'Tỵ' };
    add('Dịch Mã', dm[group]);

    const dao: Record<string, string> = { 'Thủy': 'Dậu', 'Hỏa': 'Mão', 'Kim': 'Ngọ', 'Mộc': 'Tý' };
    add('Đào Hoa', dao[group]);

    const tuong: Record<string, string> = { 'Thủy': 'Tý', 'Hỏa': 'Ngọ', 'Kim': 'Dậu', 'Mộc': 'Mão' };
    add('Tướng Tinh', tuong[group]);

    const kiep: Record<string, string> = { 'Thủy': 'Tỵ', 'Hỏa': 'Hợi', 'Kim': 'Dần', 'Mộc': 'Thân' };
    add('Kiếp Sát', kiep[group]);

    const hoa: Record<string, string> = { 'Thủy': 'Thìn', 'Hỏa': 'Tuất', 'Kim': 'Sửu', 'Mộc': 'Mùi' };
    add('Hoa Cái', hoa[group]);

    const muu: Record<string, string> = { 'Thủy': 'Tuất', 'Kim': 'Mùi', 'Hỏa': 'Thìn', 'Mộc': 'Sửu' };
    add('Mưu Tinh', muu[group]);

    const tai: Record<string, string> = { 'Thủy': 'Ngọ', 'Hỏa': 'Tý', 'Kim': 'Mão', 'Mộc': 'Dậu' };
    add('Tai Sát', tai[group]);

    const vong: Record<string, string> = { 'Thủy': 'Hợi', 'Hỏa': 'Tỵ', 'Kim': 'Thân', 'Mộc': 'Dần' };
    add('Vong Thần', vong[group]);
  }

  const mIdx = CHI.indexOf(mChi);
  if (mIdx !== -1) {
    const ty = CHI[(mIdx - 1 + 12) % 12];
    add('Thiên Y', ty);
  } else {
    add('Thiên Y', '-');
  }

  const muaMap: Record<string, string> = {
...
export function getThienCan(quaiName: string, isNgoaiQuai: boolean): string {
...
export function calculateHexagramData(lines: number[], cal: CalendarData, methodText: string, formattedDate: string): HexagramData {
  ...
  const safeCal = cal || {
    ...
    nam: { can: 'Giáp', chi: 'Tý' },
    gio: { can: 'Giáp', chi: 'Tý' },
    tuanKhong: ['Giáp', 'Ngọ'] as [string, string],
    tietKhi: ''
  };
